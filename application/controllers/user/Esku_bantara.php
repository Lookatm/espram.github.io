...
class Esku_bantara extends CI_Controller
{
    ...
    public function kenaikan_tkt($id_usr)
    {
        ...
        $data['user'] = $this->db->get_where('user_', ['username_' =>
        $this->session->userdata('username_')])->row_array();
        $data['edit'] = $this->sku_bm->edit_form($id_usr);
        $data['tempuh'] = $this->sku_bm->data_tempuh($id_usr);
        $this->load->view('user/tamu/esku/kenaikan_tingkat', $data);
    }
    public function do_unduh()
    {
        ...
        force_download('./assets/form/form_bantara.pdf', null);
    }
    public function unggah_file($id_usr)
    {
        ...
        if ($cek_nim > 0) {
            ...
            redirect('user/esku_bantara/kenaikan_tkt/' . $id_usr);
        } else {
    ...
    public function edit_unggah($id_siswa)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->userdata('username_')])->row_array();
            $this->session->set_flashdata('msg', '<div class="alert alert-danger" role="alert">Gagal Menyimpan</div>');
            redirect('user/esku_bantara/kenaikan_tingkat/' . $id_siswa);
        } else {
            ...
            if ($unggah) {
                ...
                if (!$this->upload->do_upload('unggah')) {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger" role="alert">Gagal menyimpan</div>');
                    redirect('user/esku_bantara/kenaikan_tkt/' . $id_siswa);
                } else {
                    $unggah = $this->upload->data('file_name');
                    $tanggal_ = $this->input->post('tanggal_');
                    $id_siswa = $this->input->post('id_siswa');
                    ...
                    $this->sku_bm->update_file($id_siswa, $data);
    ...
    function viewSoal($id_sku)
    {
        ...
        $data['jawab_'] = $this->sku_bm->view_soal($id_sku);
        $data['edit'] = $this->sku_bm->viewEdit($id_sku);
        $this->load->view('user/tamu/esku/soal_', $data);
    }
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($jawaban_) {
                ...
                if (!$this->upload->do_upload('jawaban_')) {
                    ...
                    $this->sku_bm->updateJawaban($id_tempuh, $data);
    ...
    public function tempuh_ulang($id_siswa)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->userdata('username_')])->row_array();
            $this->session->set_flashdata('msg', '<div class="alert alert-danger" role="alert">Gagal Menyimpan</div>');
            redirect('user/esku_bantara/viewNilai/' . $id_siswa);
        } else {
            ...
            if ($jawaban_) {
                ...
                if (!$this->upload->do_upload('jawaban_')) {
                    ...
                    $this->sku_bm->updateUlang($id_siswa, $data);
    ...
    function edit_skub($id_sku)
    {
        ...
        $sku_ = $this->sku_bm->edit_skub($id_sku);
        $data = array(
            'sku_'                => $sku_,
    ...
    function viewNilai($id_usr)
    {
        ...
        $this->load->view('user/tamu/esku/hasil_esku', $data);
    }
    function viewNilailulus($id_usr)
    {
        ...
        $data['detail_'] = $this->sku_bm->viewDetail($id_usr);
        $this->load->view('user/tamu/esku/hasil_eskululus', $data);
    }
    function viewUlang($id_tempuh)
    {
        ...
        $data['jawab_'] = $this->sku_bm->viewUlang($id_tempuh);

        $this->load->view('user/tamu/esku/edit_jawab', $data);
    }
    function detail_nilai($id_tempuh)
    {
        ...
        $this->load->view('user/tamu/esku/detailNilai', $data);
    }
}